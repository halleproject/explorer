...
func (db *Database) QueryTxsByAddress(q_address string, q_addressContract string, before int, after int, limit int) ([]schema.Transaction, error) {
	...
	if q_addressContract == "" {
		...
		case before > 0:
			err = db.Model(&txs).
				Where("(from_address = ? OR to_address = ?) AND id < ?  And to_address != 'not_erc20_method' ", q_address, q_address, before).
				Limit(limit).
				Order("id DESC").
				Select()
		case after >= 0:
			err = db.Model(&txs).
				Where("(from_address = ? OR to_address = ?) AND id > ?  And  to_address != 'not_erc20_method'", q_address, q_address, after).
				Limit(limit).
				Order("id ASC").
				Select()
		default:
			err = db.Model(&txs).
				Where("(from_address = ? OR to_address=?)  And to_address != 'not_erc20_method' ", q_address, q_address).
				Limit(limit).
				Order("id DESC").
				Select()
		}
